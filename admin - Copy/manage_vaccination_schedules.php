...
$sql = "SELECT vaccinationschedules.id, 
               ...
               vaccinationschedules.reminder_sent
            ...
            <?php while ($row = $result->fetch_assoc()) { ?>
                <tr>
                    ...
                    <td>
                        ...
                        <?php } else { ?>
                            <a href="../send_reminder.php?schedule_id=<?php echo $row['id']; ?>" class="action-btn" style="background-color: #ffc107; color: black;">
                                Send Reminder
                            </a>
                        <?php } ?>
                    </td>
                    <td>
                      <?php if ($row['status'] == 'Scheduled' && $row['reminder_sent'] == 1) { ?>
    <a href="mark_vaccination_completed.php?id=<?php echo $row['id']; ?>" class="action-btn mark-completed-btn">
        Mark Completed
    </a>
<?php } ?>
                        <a href="delete_vaccination_schedule.php?id=<?php echo $row['id']; ?>" class="action-btn delete-btn">Delete</a>
                    </td>
                </tr>
            <?php } ?>
        </table>
